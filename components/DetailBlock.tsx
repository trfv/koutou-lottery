import { LotteryDetail } from 'interfaces';
...
const DetailBlock = ({ detail }: Props) => {
  ...
  return (
    <div>
      <div style={{ textAlign: "center", cursor: "pointer" }} onClick={handleTitleClick}><h3>{building} {institution}</h3></div>
      {lotteries.map((lottery, index) => (
        <div key={index} style={{ display: "flex" }}>
          <div>
            {lottery.header.map((text, i, ) => 
              (i === 0 && index === 0 || i > 0) && (
                <DetailLine key={i} cell={{ rowId: `${index + 1}-${i}`, colId: '0' }}>{text}</DetailLine>
              )
            )}
          </div>
          {lottery.body.map((row, j) => 
            <div key={j}>
              {row.map((td, k, ) => 
                (k === 0 && index === 0 || k > 0) && (
                  <DetailLine key={k} cell={{ rowId: `${index + 1}-${k}`, colId: (j + 1).toString() }}>{td.includes("img") ? "-" : td}</DetailLine>
                )
              )}
            </div>
          )}
